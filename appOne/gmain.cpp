...
7:20
再生中
「Ｃ言語で表示された豚」と「Ｃ言語で表示した人」
Eijo Takagi
2

8:27
再生中
Ｃ言語で豚を移動して、回して、大きくする
Eijo Takagi
3

7:10
再生中
キーボードで豚を移動して、回して、変身させる
Eijo Takagi
4
...
void loading(struct DATA* d) {
    d->titleImg = loadImage("assets\\titlr.png");
...
void play(struct DATA* d) {
    draw(d);
    if (isTrigger(KEY_A)) { d->rabit.px += -d->rabit.vx; }
    if (isTrigger(KEY_D)) { d->rabit.px += d->rabit.vx; }
